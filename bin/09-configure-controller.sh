...
cmd="ansible-playbook -i ${sandbox}/hosts.yml  ${playbook} -vvv -e \"password=${AVI_DEMO_CONTROLLER_PASSWORD} controller_ip=${AVI_DEMO_CONTROLLER_IP} avi_config_state=present default_avi_password=${AVI_DEMO_DEFAULT_PASSWORD} kube_master_node=${AVI_DEMO_MASTER_IP} project_name=${AVI_DEMO_PROJECT_NAME} region_name=${AVI_DEMO_REGION}\""
echo "Executing ${cmd}"
eval ${cmd}
